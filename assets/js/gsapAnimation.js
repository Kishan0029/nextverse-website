(function ($) {
    ...
    var serviceScroll = () => {
        ...
        const initDesktop = () => {
            ...
            $mainScrolls.find(".action").on("click", function () {
                if (!stInstance) return;
                
                const index = $mainScrolls.index($(this).closest(".wg-service-2"));
                const isLeftArrow = $(this).find("i").hasClass("icon-arrow-long-left");
                
                let targetStep = index;
                if (isLeftArrow) {
                    targetStep = Math.max(0, index - 1);
                } else {
                    targetStep = Math.min(totalSteps, index + 1);
                }
                
                const targetScroll = stInstance.start + (targetStep / totalSteps) * (stInstance.end - stInstance.start);
                ...
                gsap.to(window, {
                    ...
                    onComplete: () => {
                        isNavigating = false;
                        // manually force update on the scroll trigger to ensure states match
                        stInstance.update();
                    }
                });
            });
    ...
    $(window).on("load", function () {
        // Removed hash scroll logic to prevent redirecting to contact section on refresh
        // Browser will now handle default scroll restoration
    });
})(jQuery);
